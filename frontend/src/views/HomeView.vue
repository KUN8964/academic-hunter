...
// ── Guest flow: enter API key → expand topic → see results ──

const step = ref<'enter' | 'topic' | 'results'>('enter')
...
<template>
  <div class="min-h-[80vh] flex flex-col items-center justify-center px-6">
    ...
    <div v-if="step === 'enter'" class="w-full max-w-md text-center">
      ...
      <div class="mt-8 pt-6 border-t border-zinc-800 space-y-3">
        ...
        <router-link to="/register"
          class="block w-full py-2.5 border border-zinc-700 hover:border-zinc-500 text-zinc-300 rounded-lg text-sm transition">
          注册账户
        </router-link>
    ...
    <!-- Step: Results -->
    <div v-if="step === 'results'" class="w-full max-w-lg">
      ...
      <div v-if="keywords.length" class="mb-6">
        ...
        <div class="flex flex-wrap gap-2">
          <span v-for="kw in keywords" :key="kw"
            class="px-2.5 py-1.5 bg-zinc-800 text-zinc-200 text-sm rounded-lg">{{ kw }}</span>
        </div>
      </div>
      ...
      <div v-if="subfields.length" class="mb-6">
        ...
        <div class="flex flex-wrap gap-2">
          <span v-for="sf in subfields" :key="sf"
            class="px-2.5 py-1.5 bg-zinc-800 text-zinc-200 text-sm rounded-lg">{{ sf }}</span>
        </div>
      </div>
      ...
      <div v-if="researchers.length" class="mb-8">
        ...
        <div class="space-y-2">
          <div v-for="r in researchers" :key="r.name"
            class="p-3 bg-zinc-900 border border-zinc-800 rounded-lg">
            <span class="text-white text-sm">{{ r.name }}</span>
            <span v-if="r.s2_author_id || r.orcid" class="text-zinc-500 text-xs ml-2">
              {{ r.orcid ? 'ORCID' : 'S2' }}: {{ r.orcid || r.s2_author_id }}
            </span>
          </div>
        </div>
      </div>

      <div class="bg-amber-950/20 border border-amber-900/30 rounded-lg p-4 mb-6">
        <p class="text-amber-400 text-sm">
          以上为即时分析结果。注册账户可获得：每日自动简报、论文可信度评分、AI 中文摘要等持久化功能。
        </p>
      </div>
      ...
      <div class="flex gap-3">
        ...
        <router-link to="/register"
          class="flex-1 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium text-center transition">
          注册以保存订阅 →
        </router-link>
      </div>
    </div>
